...
evidence with PDDL action schemas, adds every positively producible predicate
such as `clear(X)` to the target universe, certifies candidate branches, and
asks Clingo to select a minimum-cost feasible library whose internal calls are
implemented within the generated candidate language.
...
Rerunning the released natural-language translation stage additionally requires
the optional OpenAI-compatible transport:
...
The fixed libraries, one canonical five-seed case record, paired atomic and
temporal ablations, external planning references, Raw MOOSE extension results,
certificate challenges, benchmark-provenance compatibility certificate,
distributions, and SHA-256 manifest are under
...
After materializing the PDDL corpus, regenerate the machine-readable aggregate
and the reported LaTeX tables with the conference-neutral generator:
...
The complete environment, registered parameters, external dependency
revisions, and experimental commands are listed in
[REPRODUCING.md](REPRODUCING.md) and the versioned data release documentation.

Run the static source check with:

```bash
PYTHONDONTWRITEBYTECODE=1 uv run ruff check src scripts
```

## Citation and Public Artifact

The public code-and-data artifact is <https://github.com/daidaibunny/gp2pl>.
Use [CITATION.cff](CITATION.cff) for the framework and the
[TEG dataset citation](paper_artifacts/temporal_goal_benchmark/v1/CITATION.cff)
when using the released temporal benchmark.
