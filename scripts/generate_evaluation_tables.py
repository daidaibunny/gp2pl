...
def build_evaluation_result_dataset(
	*,
	...
	if not execution_benchmark_sha256 and benchmark_compatibility_file is not None:
		compatibility = _read_json(
			Path(benchmark_compatibility_file).expanduser().resolve(),
		)
		execution_benchmark_sha256 = str(
			compatibility.get("execution_benchmark_sha256") or "",
		)
	if not execution_benchmark_sha256:
		execution_benchmark_sha256 = benchmark_sha256
