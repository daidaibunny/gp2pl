...
class LTLfToDFA:
    ...
    def _resolve_mona_runtime() -> Tuple[str, Dict[str, str]]:
        ...
        repository_mona_root = PROJECT_ROOT / ".external" / "mona-1.4"
        candidate_commands = [
            explicit_mona,
            shutil.which("mona") or "",
            str(repository_mona_root / "Front" / "mona"),
            str(repository_mona_root / "Front" / ".libs" / "mona"),
        ]
        mona_command = next(
            (
                candidate
        ...
        if not mona_command:
            raise FileNotFoundError(
                "mona executable not found. Run scripts/setup_mona.sh, set "
                "MONA_BIN, or add mona to PATH.",
            )
        ...
        mona_path = Path(mona_command).resolve()
        if mona_path.parent.name == "Front":
            selected_mona_root = mona_path.parent.parent
        elif (
            mona_path.parent.name == ".libs"
            and mona_path.parent.parent.name == "Front"
        ):
            selected_mona_root = mona_path.parent.parent.parent
        else:
            selected_mona_root = None
        library_dirs = (
            [
                str(selected_mona_root / component / ".libs")
                for component in ("Mem", "BDD", "DFA", "GTA")
            ]
            if selected_mona_root is not None
            else []
        )
