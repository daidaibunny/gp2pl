...
def build_temporal_benchmark_release(
	*,
	...
	public_sanitization = write_sanitized_source_archive(
		source_archive=public,
		destination_archive=published_public,
	)
	private_sanitization = write_sanitized_source_archive(
		source_archive=private,
		destination_archive=published_private,
	)
	...
	with workspace_context as temporary:
		...
		"sealed_input_archive_sanitization": {
			"public_handoff_excluded_member_count": len(
				public_sanitization["excluded_members"],
			),
			"private_validation_excluded_member_count": len(
				private_sanitization["excluded_members"],
			),
		},
...
def write_portable_delivery_archive(
	*,
	...
	def portable_content(member_name: str, content: bytes) -> bytes:
		is_json_metadata = member_name.endswith((".json", ".jsonl"))
		if member_name.endswith(".jsonl"):
			transformed = _portable_jsonl_metadata(
				content,
				member_name=member_name,
				benchmark_id=benchmark_id,
			)
		elif member_name.endswith(".json"):
			transformed = _portable_json_metadata(
				content,
				member_name=member_name,
				benchmark_id=benchmark_id,
			)
			...
			transformed = content
		if is_json_metadata and (
			transformed != content
			or _contains_portable_metadata_field(transformed, member_name)
		):
			normalized_files.add(member_name)
		if member_name in required_metadata:
			payload = json.loads(transformed.decode("utf-8"))
			if not isinstance(payload, dict):
				raise ValueError(f"Expected JSON object in {member_name!r}.")
			if member_name.endswith("run_config.json"):
				...
				raise ValueError("Portable validation summary normalization failed.")
			normalized_files.add(member_name)
		return transformed

	report = _write_sanitized_archive(
		source_archive=source,
		destination_archive=destination,
		transform_content=portable_content,
	)
...
def write_sanitized_source_archive(
	*,
	source_archive: str | Path,
	destination_archive: str | Path,
) -> dict[str, object]:
	"""Write a deterministic sealed-input archive without release byproducts."""

	return _write_sanitized_archive(
		source_archive=Path(source_archive).resolve(),
		destination_archive=Path(destination_archive).resolve(),
		transform_content=lambda _member_name, content: content,
	)
...
def _write_sanitized_archive(
	*,
	...
	transform_content: Callable[[str, bytes], bytes],
) -> dict[str, object]:
	"""Copy a tarball deterministically while excluding non-release members."""
	...
	with tempfile.NamedTemporaryFile(
		dir=destination_archive.parent,
		prefix=f".{destination_archive.name}.",
		delete=False,
	) as temporary_file:
		temporary_path = Path(temporary_file.name)
	try:
		with (
			tarfile.open(source_archive, mode="r:gz") as source_bundle,
			...
			for source_member in sorted(
				...
				if _is_platform_metadata(member_path) or _is_transient_validation_diagnostic(
					member_path,
				):
					excluded_members.add(source_member.name)
					continue
				if member_path.is_absolute() or ".." in member_path.parts:
				...
				if source_member.isfile():
					...
					content = transform_content(source_member.name, extracted.read())
		...
		os.replace(temporary_path, destination_archive)
	finally:
	...
	return {
		...
		"published_sha256": _sha256(destination_archive),
	}
...
def _is_transient_validation_diagnostic(path: Path) -> bool:
	"""Return whether a delivery member is an unsuccessful validation byproduct."""
